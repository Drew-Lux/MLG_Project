...
BEST_MODEL_KEY = "decision_tree_model.pkl"
...
fig.suptitle("K-Means Validation — Elbow & Silhouette",
...
stage_pivot = stage_dist.pivot(
    index="cluster", columns=target_col, values="count").fillna(0)
...
if len(set(cluster_names.values())) < 3:
    ...
    risk_labels = ["Low Risk Lifestyle",
                   "Moderate Risk Lifestyle", "High Risk Lifestyle"]
...
profile_z = cluster_profile.apply(
...
ax.set_title("Cluster Feature Profiles (Annotated with Raw Means)",
             fontsize=13, fontweight="bold")
ax.set_xticklabels(
    [cluster_names.get(i, f"C{i}") for i in range(3)], rotation=15, ha="right")
plt.tight_layout()
...
ax.set_title("Patient Lifestyle Clusters — PCA Projection",
             fontsize=13, fontweight="bold")
...
if not os.path.exists(model_path):
    ...
    mean_shap_df.to_csv(os.path.join(
        OUTPUT_DIR, "shap_classification_importance.csv"), index=False)

    # ── Bar plot ──────────────────────────────────────────────────────────────
    top_n = min(15, len(mean_shap_df))
    fig, ax = plt.subplots(figsize=(9, 5))
    top_df = mean_shap_df.head(top_n)
    ax.barh(top_df["feature"][::-1], top_df["mean_|SHAP|"]
            [::-1], color="#E63946", alpha=0.85)
    ...
    if isinstance(shap_vals, list):
        ...
        plt.title(
            f"SHAP Summary - Class: {label_encoder.classes_[best_class]}", fontweight="bold")
    else:
        shap.summary_plot(
            shap_vals,
            X_sample_scaled,
...
if isinstance(shap_clust, np.ndarray) and shap_clust.ndim == 3:
    ...
    shap_clust_mean = np.mean([np.abs(s) for s in shap_clust], axis=0).mean(
        axis=0)  # → (n_features,)
else:
    shap_clust_mean = np.abs(shap_clust).mean(axis=0)        # binary fallback

print(
    f"  SHAP output shape check — features: {len(feature_cols)}, shap values: {len(shap_clust_mean)}")
...
mean_shap_clust_df.to_csv(os.path.join(
    OUTPUT_DIR, "shap_cluster_importance.csv"), index=False)
...
for cid in range(3):
    ...
    for feat in top_features:
        ...
        if "bmi" in f_lower:
            if val > 27:
                recs.append(
                    f"⚠ BMI is elevated ({val:.1f}). Recommend structured weight-management plan and dietitian referral.")
            else:
                recs.append(
                    f"✔ BMI is within acceptable range ({val:.1f}). Encourage maintenance through regular physical activity.")
        elif "glucose" in f_lower or "blood_glucose" in f_lower:
            if val > 140:
                recs.append(
                    f"⚠ Average blood glucose is high ({val:.1f} mg/dL). Immediate dietary carbohydrate review and HbA1c monitoring advised.")
            elif val > 100:
                recs.append(
                    f"⚠ Blood glucose is borderline ({val:.1f} mg/dL). Recommend reducing refined sugar intake and 30-min daily walks.")
        elif "hba1c" in f_lower or "a1c" in f_lower:
            if val > 6.5:
                recs.append(
                    f"⚠ HbA1c is in diabetic range ({val:.2f}%). Refer to endocrinologist and start medication review.")
            elif val > 5.7:
                recs.append(
                    f"⚠ HbA1c suggests pre-diabetes ({val:.2f}%). Begin lifestyle intervention programme immediately.")
        elif "exercise" in f_lower or "physical" in f_lower or "activity" in f_lower:
            if val < 3:
                recs.append(
                    f"⚠ Low physical activity ({val:.1f} days/week). Target ≥150 min/week moderate aerobic exercise (ADA guidelines).")
            else:
                recs.append(
                    f"✔ Physical activity is adequate ({val:.1f} days/week). Encourage consistency.")
        elif "age" in f_lower:
            if val > 50:
                recs.append(
                    f"ℹ Average age is {val:.0f}. Prioritise annual screening and cardiovascular risk co-assessment.")
        elif "diet" in f_lower:
            recs.append(
                f"ℹ Diet score is {val:.1f}. Review nutritional counselling access for this group.")
        elif "stress" in f_lower or "sleep" in f_lower:
            if val > 6:
                recs.append(
                    f"⚠ High stress/poor sleep score ({val:.1f}). Introduce mindfulness / sleep-hygiene workshops.")
        elif "smoking" in f_lower:
            if val > 0.3:
                recs.append(
                    f"⚠ Notable smoking prevalence ({val:.2f}). Provide cessation support and counselling resources.")
        elif "alcohol" in f_lower:
            if val > 0.3:
                recs.append(
                    f"⚠ Notable alcohol usage ({val:.2f}). Provide cessation support and counselling resources.")
        else:
            recs.append(
                f"ℹ Feature '{feat}' (mean={val:.2f}) is influential for this segment. Clinical review recommended.")

    if not recs:
        recs = [
            "No strong numerical signals detected — review categorical features manually."]
...
rec_df.to_csv(os.path.join(
    OUTPUT_DIR, "actionable_recommendations.csv"), index=False)
print(
